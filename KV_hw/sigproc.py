    ...
    if X2 is None: #DTW distance of X1 objects => N(N-1)/2
        ...
        for i in range(X.shape[0]-1):
            for j in range(i+1, X.shape[0]):
                ...
                R[i][j] = dtw(a, b, fines=fines, d=d, ver=ver)[0]
                R[j][i] = R[i][j]
...
#@interactive
def dtw_based(a, b, fines=np.array([5, 1, 1]), angles=None,
        metric='euclidean', d=None, ver=1):
    """
    Метрика двух сигналов (corporate).
    Описание параметров:
        a, b - numpy.array, пара сигналов, вектор столбец Mx2 и Nx2;
        fines - numpy.array, штраф за совпадение ai, bj,
                              за растяжение по i (bj фиксир),
                              за сжатие по i (ai фиксир);
        angles - tuple(numpy.array, -//-), ((M-1), (N-1)),
             углы между векторами (из соседних точек) и горизонтом (arctg);
        metric - метрика сходства (при 0 значении) между векторами измерений;
    ...
    while i > 0 and j > 0:
        ...
        i = i - (min_ind != 2)
        j = j - (min_ind != 1)
        s = min(a_ang[i-1] - b_ang[j-1], 2*np.pi - a_ang[i-1] + b_ang[j-1])
        ang_s += s*s
        K += 1
        #path = [[i, j]] + path
    #if i > 0 or j > 0:
...
def m_distance_features_based(X1, fines, X2=None, d=None, ver=1, based=False):
    ...
    Angles1 = np.ma.masked_array(np.zeros((X1.shape[0], 392)), mask=X1.mask[:, 0, :])
    Angles1[AnglesX != 0] = np.arctan(AnglesY[AnglesX != 0] / AnglesX[AnglesX != 0])
    if X2 is None: #DTW distance of X1 objects => N(N-1)/2
        X = X1
        R = np.zeros((X.shape[0], X.shape[0], 3), dtype=np.float64)
        for i in range(X.shape[0]-1):
            for j in range(i+1, X.shape[0]):
                #print(i, j)
                a = X[i].compressed().reshape(2, -1).T
                b = X[j].compressed().reshape(2, -1).T
                ang1 = Angles1[i].compressed()
                ang2 = Angles1[j].compressed()
                R[i][j] = dtw_based(a, b, angles=(ang1, ang2), fines=fines, d=d, ver=ver)
                R[j][i] = R[i][j]
    else: #DTW distance of X1 to X2 objects => N*M
        Angles2 = X2[..., 1:] - X2[..., :-1] #(vectors)
        AnglesX = Angles2[:, 0, :].copy()
        AnglesY = Angles2[:, 1, :].copy()
        Angles2 = np.ma.masked_array(np.zeros((X2.shape[0], 392)), mask=X2.mask[:, 0, :])
        Angles2[AnglesX != 0] = np.arctan(AnglesY[AnglesX != 0] / AnglesX[AnglesX != 0])
        R = np.zeros((X1.shape[0], X2.shape[0], 3), dtype=np.float64)
        for i in range(X1.shape[0]):
            for j in range(X2.shape[0]):
                a = X1[i].compressed().reshape(2, -1).T
                b = X2[j].compressed().reshape (2, -1).T
                ang1 = Angles1[i].compressed()
                ang2 = Angles2[j].compressed()
                R[i][j] = dtw_based(a, b, angles=(ang1, ang2), fines=fines, d=d, ver=ver)
    return R
